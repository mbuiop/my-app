...
// ============================================
// ارسال ایمیل - اگه SMTP تنظیم شده باشه واقعاً می‌فرسته، وگرنه فقط تو کنسول سرور
// لاگ می‌کنه (برای توسعه/تست بدون داشتن سرویس ایمیل واقعی)
// برای فعال کردن ارسال واقعی: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM
// ============================================
...
if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
        ...
        console.error('⚠️ nodemailer نصب نیست یا خطا داشت، ایمیل‌ها فقط لاگ می‌شن:', e.message);
    }
} else {
    console.log('ℹ️  SMTP تنظیم نشده - کدهای بازیابی رمز فقط تو کنسول سرور لاگ می‌شن (برای production باید SMTP_HOST/SMTP_USER/SMTP_PASS رو ست کنی)');
}
...
async function sendEmail(to, subject, text) {
    ...
    console.log(`📧 [ایمیل شبیه‌سازی‌شده - SMTP تنظیم نشده] به: ${to} | موضوع: ${subject}\n${text}`);
    return true;
}
...
// ============================================
// بازیابی رمز عبور - کد ۶ رقمی یک‌بارمصرف با انقضای ۱۵ دقیقه‌ای
// ============================================
app.post('/api/auth/forgot-password', authLimiter, async (req, res) => {
    try {
        ...
        // برای جلوگیری از لو رفتن اینکه چه ایمیلی ثبت‌نام کرده، همیشه success برمی‌گردونیم
        if (!user || !user.email) {
        ...
        await sendEmail(user.email, 'بازیابی رمز عبور یارِ من',
            `کد بازیابی رمز عبورت: ${code}\nاین کد تا ۱۵ دقیقه دیگه معتبره. اگه درخواست نداده بودی، این پیام رو نادیده بگیر.`);
...
app.post('/api/admin/broadcast', isAdmin, async (req, res) => {
    try {
        ...
        // کاربرها بین ۱۵۰ شارد (schema) پخشن؛ هر شارد رو جدا و موازی می‌خونیم و insert می‌کنیم
