...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body.dark-theme .header .menu-icon,
        body.dark-theme .header .header-right i {
            color: #ffffff;
        }
        ...
        body.dark-theme .story-username {
            color: #ffffff;
        }
        ...
        body.dark-theme .gallery-item .post-actions {
            background: #1e1e1e;
            border-top-color: #2c2c2c;
        }
        body.dark-theme .gallery-item .post-actions .action-btn {
            color: #ffffff;
        }
        body.dark-theme .gallery-item .post-actions .action-btn:hover {
            background: #2c2c2c;
        }
        body.dark-theme .post-views {
            color: #888;
        }
        ...
        body.dark-theme .bottom-nav button {
            color: #ffffff;
        }
        body.dark-theme .bottom-nav button i {
            color: #ffffff;
        }
        ...
        body.dark-theme .profile-stats .stat .number {
            color: #ffffff;
        }
        body.dark-theme .profile-stats .stat .label {
            color: #888;
        }
        body.dark-theme .profile-gallery {
            background: #121212;
        }
        body.dark-theme .profile-post .post-actions {
            background: #1e1e1e;
            border-top-color: #2c2c2c;
        }
        body.dark-theme .profile-post .post-actions .action-btn {
            color: #ffffff;
        }
        ...
        body.dark-theme .profile-header .close-profile {
            color: #ffffff;
        }
        body.dark-theme .side-menu {
            background: #1e1e1e;
        }
        body.dark-theme .side-menu .menu-header {
            border-bottom-color: #2c2c2c;
        }
        body.dark-theme .side-menu .menu-header h3 {
            color: #ffffff;
        }
        body.dark-theme .side-menu .menu-header .close-menu {
            color: #ffffff;
        }
        body.dark-theme .side-menu .menu-item {
            border-bottom-color: #2c2c2c;
            color: #ffffff;
        }
        body.dark-theme .side-menu .menu-item:hover {
            background: #2c2c2c;
        }
        body.dark-theme .side-menu .menu-item i {
            color: #ffffff;
        }
        ...
        body.dark-theme .chat-message.own {
            background: #0095f6;
            color: #ffffff;
        }
        ...
        body.dark-theme .chat-user {
            color: #ffffff;
        }
        body.dark-theme .chat-user:hover {
            background: #2c2c2c;
        }
        ...
        body.dark-theme .modal-header .close-modal {
            color: #ffffff;
        }
        ...
        body.dark-theme .modal-footer input {
            background: #2c2c2c;
            color: #ffffff;
            border-color: #3c3c3c;
        }
        ...
        body.dark-theme .share-modal-header .close-share {
            color: #ffffff;
        }
        ...
        body.dark-theme .share-option .share-name {
            color: #ffffff;
        }
        ...
        body.dark-theme .upload-header .close-upload {
            color: #ffffff;
        }
        ...
        body.dark-theme .upload-container p {
            color: #888;
        }
        body.dark-theme .upload-caption textarea {
            background: #2c2c2c;
            color: #ffffff;
            border-color: #3c3c3c;
        }
        body.dark-theme .follow-modal-content {
            background: #1e1e1e;
        }
        body.dark-theme .follow-modal-header {
            border-bottom-color: #2c2c2c;
        }
        body.dark-theme .follow-modal-header h3 {
            color: #ffffff;
        }
        body.dark-theme .follow-modal-header .close-follow {
            color: #ffffff;
        }
        body.dark-theme .follow-item {
            border-bottom-color: #2c2c2c;
        }
        body.dark-theme .follow-item .follow-name {
            color: #ffffff;
        }
        .header {
            background: white;
        ...
        .gallery-item .explore-post-actions .action-btn:hover {
            background: rgba(255, 255, 255, 0.2);
        }
        .gallery-item .explore-post-actions .action-btn.liked i {
            color: #ff4757;
        }
        .bottom-nav {
            position: fixed;
        ...
        .profile-post .image-container {
            width: 100%;
            height: 100%;
            position: relative;
        }
        .profile-post .image-container img {
            width: 100%;
            height: 100%;
            object-fit: cover;
        }
        ...
        .profile-post .image-container:hover .profile-post-overlay {
            opacity: 1;
        }
        .profile-post .image-container .profile-post-overlay span {
            display: flex;
            align-items: center;
            gap: 5px;
            font-size: 14px;
        ...
        .profile-post .image-container .profile-post-overlay i {
            font-size: 16px;
        }
        .upload-page {
            display: none;
        ...
        .share-option .share-icon.telegram {
            background: #0088cc;
        }
...
<body>
    ...
    <script>
        ...
        socket.on('receive-message', (data) => {
            displayChatMessage(data.userId, data.userName, data.text, data.timestamp);
        });
        ...
        socket.on('history-messages', (messages) => {
            ...
            messages.forEach(msg => {
                displayChatMessage(msg.userId, msg.userName, msg.text, msg.timestamp);
            });
        });

        function startChat(targetUserId, targetUserName) {
            currentChatUser = targetUserId;
            const roomId = [USER_ID, targetUserId].sort().join('_');
            currentChatRoom = roomId;

            document.getElementById('chatTitle').textContent = '💬 چت با ' + targetUserName;
        ...
        function sendChatMessage() {
            ...
            displayChatMessage(USER_ID, USER_NAME, text, Date.now());
            input.value = '';
        }

        function displayChatMessage(userId, userName, text, timestamp) {
            const messagesDiv = document.getElementById('chatMessages');
            const emptyMsg = messagesDiv.querySelector('.chat-empty');
            if (emptyMsg) emptyMsg.remove();

            const div = document.createElement('div');
            div.className = 'chat-message' + (userId === USER_ID ? ' own' : '');

            const time = timestamp ? new Date(timestamp).toLocaleTimeString('fa-IR') : '';

            div.innerHTML = `
                <div class="msg-user">${userId === USER_ID ? 'شما' : userName}</div>
        ...
        function renderUsersList(users) {
            ...
            if (!users || Object.keys(users).length === 0) {
                list.innerHTML = '<div style="padding:10px 16px;color:#8e8e8e;">هیچ کاربری آنلاین نیست</div>';
                return;
            }
            ...
            for (const [userId, data] of Object.entries(users)) {
                ...
                div.onclick = () => startChat(userId, data.name || 'کاربر');

                const statusClass = data.online ? 'online' : '';
                const statusText = data.online ? 'آنلاین' : 'آفلاین';

                div.innerHTML = `
                    <div class="user-avatar">
                        <img src="https://i.pravatar.cc/150?img=${Math.floor(Math.random() * 70)}" alt="user">
                    </div>
                    <div>
                        <div class="user-name">${data.name || 'کاربر'}</div>
            ...
            if (!hasUsers) {
                list.innerHTML = '<div style="padding:10px 16px;color:#8e8e8e;">هیچ کاربر دیگری آنلاین نیست</div>';
            }
        }
        ...
        function createPostElement(post) {
            ...
            div.innerHTML = `
                ...
                <div class="explore-post-actions">
                    <button class="action-btn like-btn ${isLiked ? 'liked' : ''}" data-id="${post.id}" onclick="event.stopPropagation(); handleLike('${post.id}')">
                        <i class="${isLiked ? 'fas' : 'far'} fa-heart"></i>
                        <span class="count">${post.likes || 0}</span>
                    </button>
                    <button class="action-btn comment-btn" data-id="${post.id}" onclick="event.stopPropagation(); openComments('${post.id}')">
                        <i class="far fa-comment"></i>
                        <span class="count">${(post.comments || []).length}</span>
                    </button>
                    <button class="action-btn share-btn" data-id="${post.id}" onclick="event.stopPropagation(); sharePost('${post.id}')">
        ...
        function renderStories() {
            ...
            container.innerHTML = '';
            const users = [
                { name: 'user1', img: 1 }, { name: 'user2', img: 2 }, { name: 'user3', img: 3 },
                { name: 'user4', img: 4 }, { name: 'user5', img: 5 }, { name: 'user6', img: 6 },
                { name: 'user7', img: 7 }
            ];
            ...
            users.forEach(user => {
                ...
                `;
                div.onclick = () => alert('استوری ' + user.name + ' باز شد!');
        ...
        window.handleLike = async function(postId) {
            ...
            const allLikeBtns = document.querySelectorAll('.like-btn[data-id="' + postId + '"]');
                ...
                post.comments.forEach(comment => {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="comment-content">
                            <div class="comment-username">${comment.userName || comment.userId || 'کاربر'}</div>
                            <div class="comment-text">${comment.text}</div>
                            <div class="comment-time">${comment.time || 'چند لحظه پیش'}</div>
                        </div>
                    `;
                    list.appendChild(div);
                });
        ...
        window.openPostDetail = async function(postId) {
            ...
            alert('📸 ' + (post.caption || 'بدون توضیحات') + '\\n❤️ ' + (post.likes || 0) + ' لایک\\n💬 ' + (post.comments || []).length + ' کامنت');
        };
        ...
        document.getElementById('statFollowers').addEventListener('click', function() {
            ...
            document.getElementById('followModalBody').innerHTML = `
                <div style="text-align:center;color:#8e8e8e;padding:20px;">
                    <i class="fas fa-users" style="font-size:40px;display:block;margin-bottom:15px;"></i>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('✅ App started! User:', USER_NAME, 'ID:', USER_ID);
        });
    </script>
</body>
</html>
...
server.listen(PORT, () => {
    ...
    console.log('📁 پوشه uploads برای ذخیره تصاویر');
});