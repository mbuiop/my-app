...
class RateLimiter:
    ...
    def get_user_builds_today(self, user_id):
        ...
        if REDIS_AVAILABLE:
            ...
            cache_file = os.path.join(DIRS['CACHE'], f"{key}.json")
...
class BuildQueue:
    ...
    def _start_workers(self):
        for i in range(CLUSTER_CONFIG['MAX_CONCURRENT_BUILDS']):
            t = threading.Thread(target=self._worker, daemon=True)
            t.start()
            self.worker_threads.append(t)
        logger.info(f"✅ Started {CLUSTER_CONFIG['MAX_CONCURRENT_BUILDS']} build workers")
    
    def add_build(self, user_id, file_path, file_name, chat_id, message_id, priority=5):
    ...
    def _worker(self):
        while True:
            try:
                priority, _, build_item = self.queue.get(timeout=1)
                with self.lock:
    ...
    def _process_build(self, build_item):
        ...
        try:
            ...
            build_item['callback'](build_item)
    ...
    def _save_queue_state(self):
        try:
            state_file = os.path.join(DIRS['QUEUE'], 'queue_state.json')
            with self.lock:
                state = {
...
class HealthChecker:
    ...
    def _monitor(self):
        while self.checking:
            try:
                interval = int(get_setting('health_check_interval'))
...
# ==================== دیتابیس بزرگ‌تر با Connection Pool ====================
class Database:
    def __init__(self):
        self.connections = []
        self.pool_size = CLUSTER_CONFIG['DB_POOL_SIZE']
        self._init_db()
        self._init_tables()
...
def log_error(error_type, message, user_id=None, bot_id=None):
    ...
    # حذف خطاهای قدیمی (بیشتر از ۷ روز)
...
def queue_status(message):
    ...
    concurrent = len([p for p in build_queue.processing.values() if p.get('status') == 'processing'])
    
    text = (f"⚡ **وضعیت صف ساخت ربات**\n\n"
            f"📊 در صف: {queue_len}\n"
            f"🔄 در حال پردازش: {concurrent}\n"
...
def handle_build_file(message):
    ...
    try:
        ...
        def build_callback(item):
            ...
            if result['success']:
                ...
                if user and user['referred_by']:
                    ...
                    bot.send_message(user['referred_by'], f"🎉 کمیسیون {commission:,} تومان اضافه شد")
                
                bot.edit_message_text(f"✅ ربات {bot_info['first_name']} ساخته شد!", item['chat_id'], item['message_id'])
            else:
                bot.edit_message_text(f"❌ خطا: {result.get('error')}", item['chat_id'], item['message_id'])
        ...
        build_queue.add_build(user_id, file_path, file_name, message.chat.id, status_msg.message_id)
...
def process_set_concurrent(message):
    ...
    try:
        ...
        if 5 <= concurrent <= 200:
            ...
            bot.reply_to(message, "❌ عدد بین 5 تا 200")
    except:
        bot.reply_to(message, "❌ عدد معتبر")
...
if __name__ == "__main__":
    ...
    print(f"📊 صف ساخت: {CLUSTER_CONFIG['MAX_CONCURRENT_BUILDS']} همزمان")
