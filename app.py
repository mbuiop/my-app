...
# ==================== مدیریت کلیدهای API (به دلیل طولانی بودن در بخش بعدی) ====================
# ==================== مدیریت کلیدهای API ====================
...
class YouTubeEarningBot:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ===== ثبت لینک یوتیوب =====
            ...
            # دریافت لینک‌های دیگران برای تعامل
            links = await self.db.get_links_for_view(user_id)
            ...
            # اگر لینکی برای بازدید وجود دارد، نمایش بده
            if links:
                link_text = ""
                for i, link in enumerate(links[:5], 1):
                    full_link_code = self.db.get_full_unique_code(link['unique_code'])
                    link_text += f"{i}. 🎬 {link['video_url']}\n   👤 {link.get('first_name', 'کاربر')}\n   🔑 کد: <code>{full_link_code}</code>\n\n"
                
                await update.message.reply_text(
                    f"👀 **لینک‌های برای بازدید:**\n\n{link_text}\n\n"
                    f"🔑 پس از بازدید هر ویدیو، کد یکتا آن را وارد کنید.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("✅ کد یکتا را وارد کردم", callback_data='enter_unique_code')],
                        [InlineKeyboardButton("🔙 بازگشت", callback_data='back')]
                    ]),
