...
"""
═══════════════════════════════════════════════════════════════════════════════
🚀 ربات مادر نهایی - نسخه 21.0 Ultimate
⚡ دارای: سیستم رفرال لینک ثابت + اشتراک ماهیانه + پنل مدیریت کامل + ۵۰ کتابخانه
═══════════════════════════════════════════════════════════════════════════════
"""
...
def is_channel_bot(token):
    """بررسی اینکه توکن متعلق به کانال است یا ربات"""
    try:
...
def install_library(lib_name, chat_id, msg_id):
    ...
    try:
        ...
        if process.returncode == 0:
            bot.edit_message_text(f"✅ کتابخانه {lib_name} با موفقیت نصب شد!", chat_id, msg_id)
            return True
        else:
            error = process.stderr[:200] if process.stderr else "خطای ناشناخته"
            bot.edit_message_text(f"❌ خطا در نصب {lib_name}:\n{error}", chat_id, msg_id)
            return False
    except subprocess.TimeoutExpired:
        bot.edit_message_text(f"❌ زمان نصب {lib_name} بیش از حد طول کشید!", chat_id, msg_id)
        return False
    except Exception as e:
...
def cmd_start(message):
    ...
    if len(args) > 1:
        code = args[1]
        # پیدا کردن کاربر با کد رفرال (به جز خودش)
        ...
        if users and users[0]['user_id'] != user_id:
            ...
            bot.send_message(referred_by, f"🎉 {first_name} با لینک دعوت شما وارد ربات شد!")
    
    create_user(user_id, username, first_name, message.from_user.last_name or "", referred_by)
    
    user = get_user(user_id)
    
    # ⭐ لینک رفرال ثابت و درست - با نام ربات خودت
    referral_link = f"https://t.me/{BOT_USERNAME}?start={user['referral_code']}"
    
    # متن خوش‌آمدگویی با لینک رفرال قشنگ
    text = (
        f"🚀 به ربات سازنده ربات خوش آمدید {first_name}!\n\n"
        f"👤 شناسه شما: `{user_id}`\n"
        f"🎁 کد معرف: `{user['referral_code']}`\n\n"
        f"🔗 **لینک دعوت اختصاصی شما:**\n"
        f"`{referral_link}`\n\n"
        f"📊 آمار دعوت‌ها: {user['referrals_count']} نفر\n"
        f"💰 موجودی کیف پول: {user['wallet_balance']:,} تومان\n\n"
        f"✨ با دعوت دوستان خود، از هر نفر ۷٪ کمیسیون دریافت کنید!\n"
        f"📤 برای ساخت ربات، فایل .py یا .zip خود را ارسال کنید"
    )
    
    # ارسال با دکمه شیشه‌ای برای کپی لینک
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📋 کپی لینک دعوت", callback_data=f"copy_link_{user['referral_code']}"))
    markup.add(types.InlineKeyboardButton("📤 اشتراک‌گذاری لینک", switch_inline_query=f"به ربات سازنده ربات خوش آمدید! با لینک زیر ثبت نام کنید: {referral_link}"))
    
    bot.send_message(message.chat.id, text, parse_mode='Markdown', reply_markup=get_main_menu(user_id in ADMIN_IDS))
    bot.send_message(message.chat.id, "🔗 لینک دعوت شما:", reply_markup=markup)
...
def wallet(message):
    ...
    # لینک رفرال برای نمایش در کیف پول
    referral_link = f"https://t.me/{BOT_USERNAME}?start={user['referral_code']}"
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔗 لینک دعوت من", callback_data=f"show_invite_{user['referral_code']}"))
    
    bot.send_message(message.chat.id, text, parse_mode='Markdown', reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data.startswith('show_invite_'))
def show_invite_link(call):
    code = call.data.replace('show_invite_', '')
    link = f"https://t.me/{BOT_USERNAME}?start={code}"
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📋 کپی لینک", callback_data=f"copy_link_{code}"))
    markup.add(types.InlineKeyboardButton("📤 اشتراک‌گذاری", switch_inline_query=f"به ربات سازنده ربات بپیوندید: {link}"))
    bot.send_message(call.message.chat.id, f"🔗 لینک دعوت شما:\n`{link}`", parse_mode='Markdown', reply_markup=markup)
...
def invite(message):
    ...
    text = (
        ...
        f"🔗 **لینک دعوت اختصاصی:**\n"
        f"`{referral_link}`\n\n"
        f"📊 تعداد دعوت‌ها: {user['referrals_count']} نفر\n"
        f"💰 کمیسیون هر دعوت: {get_setting('withdraw_percent')}% مبلغ اشتراک ({int(get_setting('subscription_price') * get_setting('withdraw_percent') / 100):,} تومان)\n\n"
        f"⭐ **چگونه کار می‌کند؟**\n"
        f"1️⃣ لینک خود را برای دوستان ارسال کنید\n"
        f"2️⃣ دوستان شما با لینک وارد ربات می‌شوند\n"
        f"3️⃣当他们 اشتراک ماهیانه خریداری کنند\n"
        f"4️⃣ {get_setting('withdraw_percent')}% مبلغ به کیف پول شما اضافه می‌شود\n\n"
        f"💎 هر زمان موجودی شما به {get_setting('min_withdraw'):,} تومان رسید، می‌توانید برداشت کنید."
    )
    
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("📋 کپی لینک", callback_data=f"copy_link_{user['referral_code']}"))
    markup.add(types.InlineKeyboardButton("📤 اشتراک‌گذاری", switch_inline_query=f"🤖 به ربات سازنده ربات بپیوندید!\n\nلینک دعوت: {referral_link}"))
...
def handle_build_file(message):
    ...
    try:
        ...
        # ⭐⭐⭐ بررسی اینکه توکن کانال نباشد ⭐⭐⭐
        is_channel, channel_user = is_channel_bot(token)
        if is_channel:
            bot.edit_message_text("❌ این توکن متعلق به **کانال** است!\nلطفاً توکن **ربات** خود را وارد کنید.", 
        ...
        # بررسی نهایی: حتماً ربات باشد
        if not bot_info.get('is_bot', False):
            bot.edit_message_text("❌ این توکن متعلق به **کانال** است!\nلطفاً توکن ربات واقعی وارد کنید.", 
        ...
        if result['success']:
            ...
            if user and user['referred_by']:
                ...
                bot.send_message(user['referred_by'], f"🎉 {user['first_name']} اشتراک خرید! {commission:,} تومان به کیف پول شما اضافه شد.\n🔗 لینک دعوت شما: https://t.me/{BOT_USERNAME}?start={get_user(user['referred_by'])['referral_code']}")
...
def library_menu(message):
    ...
    markup.add(types.InlineKeyboardButton("📋 همه کتابخانه‌ها", callback_data="lib_all"))
    markup.add(types.InlineKeyboardButton("🔙 بازگشت", callback_data="lib_back"))
    
    bot.reply_to(message, "📦 **کتابخانه پایتون**\nکتابخانه مورد نظر رو انتخاب کن:", parse_mode='Markdown', reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "lib_all")
def library_all(call):
    text = "📦 **لیست ۵۰ کتابخانه:**\n\n"
    for i, (name, lib) in enumerate(POPULAR_LIBRARIES.items(), 1):
        text += f"{i}. `{name}` → `{lib}`\n"
    
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 برگشت", callback_data="lib_back"))
    
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, parse_mode='Markdown', reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "lib_manual")
def library_manual(call):
    msg = bot.send_message(call.message.chat.id, "🔧 نام کتابخانه رو وارد کن:\nمثال: `requests` یا `aiogram==3.0.0`", parse_mode='Markdown')
...
def install_custom_library(message):
    ...
    if not lib:
        bot.reply_to(message, "❌ نام کتابخانه رو وارد کن")
        return
    
    status = bot.reply_to(message, f"🔄 نصب {lib}...")
    install_library(lib, message.chat.id, status.message_id)

@bot.callback_query_handler(func=lambda call: call.data.startswith('lib_') and call.data not in ['lib_manual', 'lib_all', 'lib_back'])
...
if __name__ == "__main__":
    print("=" * 80)
    print("🚀 ربات مادر نهایی - نسخه 21.0 Ultimate".center(80))
    print("=" * 80)
    ...
    print(f"📦 کتابخانه‌ها: {len(POPULAR_LIBRARIES)} عدد")
    print("✅ جلوگیری از توکن کانال: فعال")
    print("=" * 80)
