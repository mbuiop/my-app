...
class UltraScalableDatabase:
    ...
    async def get_daily_links_for_user(self, user_id: int, count: int = DAILY_INTERACTIONS_REQUIRED) -> List[Dict]:
        """دریافت لینک‌های روزانه برای کاربر - همیشه ۵ لینک نمایش داده میشه"""
        today = datetime.now().date().isoformat()
        ...
        async with self.get_connection(user_id) as conn:
            ...
            # ===== دریافت لینک‌های امروز که کاربر ندیده =====
            cursor.execute("""
            ...
            # ===== اگر لینک کافی نبود، از بقیه بگیر =====
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == 'change_language':
            ...
            if links:
                ...
                await query.edit_message_text(
                    texts['no_links_to_view'].format(count=stats['views']),
            ...
            if links:
                # ===== ثبت بازدید برای همه لینک‌ها =====
                for link in links:
                    await self.db.mark_link_viewed(user_id, link['id'])
                
                stats = await self.db.get_user_daily_stats(user_id)
                await query.edit_message_text(
                    texts['all_links_viewed'].format(count=len(links)),
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data='back')]]),
                    parse_mode=ParseMode.HTML
                )
            else:
