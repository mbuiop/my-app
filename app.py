...
"""
═══════════════════════════════════════════════════════════════════════════════
🚀 ربات مادر نهایی - نسخه 18.0 HyperSpeed Ultimate
...
CLUSTER_CONFIG = {
    ...
    'USERS_PER_MACHINE': 500,  # هر ۵۰۰ کاربر یک ماشین جدید
}
    ...
    def _init_tables(self):
        ...
        self.execute('''
            CREATE TABLE IF NOT EXISTS referral_earnings (
                ...
                created_at TIMESTAMP,
                UNIQUE(user_id, referred_user_id, bot_id)
            )
        ''')
        ...
        # ایجاد ماشین‌ها با لود بالانس ۵۰۰ نفری
        total_machines = CLUSTER_CONFIG['TOTAL_MACHINES']
...
class SecureSandbox:
    """ایزوله‌سازی کامل کد کاربران - هر کد در محیط جداگانه اجرا می‌شود"""
        ...
        for line in lines:
            if line.strip().startswith('import ') or line.strip().startswith('from '):
                ...
                if not allowed and 'telebot' not in line:
                    return False, f"⚠️ ماژول غیرمجاز: {line[:50]}"
...
# جلوگیری از ایجاد فایل
def secure_open(*args, **kwargs):
    if args[0].startswith('/') or args[0].startswith('./') or args[0].startswith('../'):
        raise PermissionError("دسترسی به فایل سیستم ممنوع است")
    return open(*args, **kwargs)

__builtins__['original_open'] = __builtins__['open']
...
'''
    ...
    def extract_project_files(self, zip_path: str, extract_to: str) -> dict:
        """استخراج تمام فایل‌های پروژه از zip"""
        files = {}
        try:
    ...
    def run_in_sandbox(self, bot_id: str, code: str, token: str, machine_id: int, project_files: dict = None) -> dict:
        """اجرای کد در محیط ایزوله با پشتیبانی از پروژه‌های چندفایلی"""
        try:
    ...
    def _monitor_resources(self, process, bot_id: str, log_file: str):
        try:
            time.sleep(2)
            while process.poll() is None:
                try:
                    ...
                    if cpu > self.MAX_CPU_PERCENT * 2:
                        with open(log_file, 'a') as f:
                            f.write(f"\n[SECURITY] CPU limit exceeded: {cpu}%\n")
                        os.killpg(os.getpgid(process.pid), signal.SIGKILL)
                        break
                    
                    if mem > self.MAX_MEMORY_MB * 1.5:
                        with open(log_file, 'a') as f:
                            f.write(f"\n[SECURITY] Memory limit exceeded: {mem}MB\n")
                        os.killpg(os.getpgid(process.pid), signal.SIGKILL)
                        break
...
# ==================== لود بالانس ۵۰۰ نفری ====================
class LoadBalancer:
    """هر ۵۰۰ کاربر روی یک ماشین مجزا"""
    ...
    def _init_machines(self):
        ...
        for i in range(1, required + 1):
            db.execute('''
                UPDATE machines SET max_users = ?, status = 'active' WHERE id = ?
            ''', (users_per_machine, i))
    
    def get_machine_for_user(self, user_id: int) -> int:
        """تعیین ماشین مناسب برای کاربر بر اساس هش"""
        users_per_machine = CLUSTER_CONFIG['USERS_PER_MACHINE']
        machine_id = ((user_id * 7919) % CLUSTER_CONFIG['TOTAL_MACHINES']) + 1
        return machine_id
...
class HyperEngine:
    ...
    def run_bot(self, bot_id, code, token, project_files=None):
        try:
            ...
            # اجرا در محیط ایزوله با پشتیبانی از پروژه
...
def add_bot(user_id, bot_id, token, name, username, file_path, folder_path=None, pid=None, machine_id=None, project_files=None):
    try:
        ...
        if user and user.get('referred_by'):
            commission = int(CARD_INFO['price'] * 0.1)  # ۱۰٪ از مبلغ
            db.execute('''
...
def get_admin_panel_menu():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("🏦 سیستم بانکی", callback_data="admin_bank_system"),
...
def cmd_start(message):
    ...
    if len(args) > 1:
        # بررسی لینک رفرال - فقط برای کاربران دیگر ثبت می‌شود
...
def withdrawal_request(message):
    ...
    markup.add(types.InlineKeyboardButton("💰 برداشت تمام موجودی", callback_data=f"withdraw_all_{balance}"))
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('withdraw_all_'))
def withdraw_all_callback(call):
    amount = int(call.data.replace('withdraw_all_', ''))
    bot.answer_callback_query(call.id)
    msg = bot.send_message(call.message.chat.id, f"مبلغ {amount:,} تومان انتخاب شد.\nلطفاً شماره کارت خود را وارد کنید:")
...
def process_card_holder(message, amount, card_number):
    ...
    if request_withdrawal(message.from_user.id, amount, card_number, card_holder):
        ...
        bot.reply_to(message, "❌ خطا در ثبت درخواست. لطفاً دوباره تلاش کنید.")
...
def install_library_menu(message):
    ...
    libs = [
        ...
        ('📦 aiogram', 'aiogram'), ('📦 jdatetime', 'jdatetime'), ('🔧 نصب دستی', 'custom')
    ]
...
def handle_build_file(message):
    ...
    try:
        ...
        if file_name.endswith('.zip'):
            ...
            # پیدا کردن فایل اصلی
            for fname in ['bot.py', 'main.py', 'run.py', '__init__.py']:
        ...
        if not token:
            bot.edit_message_text("❌ توکن ربات در کد یافت نشد\nلطفاً مطمئن شوید `token = 'YOUR_TOKEN'` در کد وجود دارد", 
        ...
        if result['success']:
            add_bot(
                ...
                file_path, None, result['pid'], result['machine_id'], project_files
            )
...
def stats(message):
    try:
        ...
        bot.send_message(message.chat.id, f"📊 خطا در دریافت آمار: {str(e)}")
...
def process_broadcast(message):
    ...
    bot.edit_message_text(f"✅ **ارسال همگانی completed!**\n\nموفق: {sent}\nناموفق: {failed}", 
...
def admin_cards_menu(call):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("➕ اضافه کردن کارت جدید", callback_data="bank_add_card"))
...
@bot.callback_query_handler(func=lambda call: call.data == "admin_bank_system")
def admin_bank_system_menu(call):
    if call.from_user.id not in ADMIN_IDS:
        bot.answer_callback_query(call.id, "⛔")
        return
    bot.answer_callback_query(call.id)
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton("➕ اضافه کردن کارت جدید", callback_data="bank_add_card"),
        types.InlineKeyboardButton("📋 لیست کارت‌ها", callback_data="bank_list_cards"),
        types.InlineKeyboardButton("🔙 بازگشت", callback_data="admin_back")
    )
    bot.send_message(call.message.chat.id, "🏦 **مدیریت سیستم بانکی**", reply_markup=markup, parse_mode='Markdown')

@bot.callback_query_handler(func=lambda call: call.data == "bank_add_card")
def bank_add_card(call):
    if call.from_user.id not in ADMIN_IDS:
        bot.answer_callback_query(call.id, "⛔")
        return
    bot.answer_callback_query(call.id)
    
    msg = bot.send_message(call.message.chat.id, "💳 **شماره کارت (۱۶ رقم):**", parse_mode='Markdown')
...
def process_add_card_number(message):
    ...
    msg = bot.reply_to(message, "👤 **نام صاحب کارت:**")
    bot.register_next_step_handler(msg, lambda m: process_add_card_holder(m, card_number))

def process_add_card_holder(message, card_number):
    if message.from_user.id not in ADMIN_IDS:
        return
    card_holder = message.text.strip()
    if len(card_holder) < 3:
    ...
    msg = bot.reply_to(message, "🏦 **نام بانک:**")
    bot.register_next_step_handler(msg, lambda m: process_add_card_bank(m, card_number, card_holder))

def process_add_card_bank(message, card_number, card_holder):
    if message.from_user.id not in ADMIN_IDS:
        return
    bank_name = message.text.strip()
    
    db.execute('''
        INSERT INTO admin_cards (card_number, card_holder, bank_name, created_at)
        VALUES (?, ?, ?, ?)
    ''', (card_number, card_holder, bank_name, datetime.now().isoformat()))
    
    bot.reply_to(message, f"✅ **کارت با موفقیت اضافه شد**\n\n💳 {card_number}\n👤 {card_holder}\n🏦 {bank_name}")

@bot.callback_query_handler(func=lambda call: call.data == "bank_list_cards")
def bank_list_cards(call):
    if call.from_user.id not in ADMIN_IDS:
        bot.answer_callback_query(call.id, "⛔")
        return
    bot.answer_callback_query(call.id)
    ...
    cards = get_admin_cards()
    text = "💳 **لیست کارت‌های بانکی**\n\n"
    for c in cards:
        text += f"🆔 {c['id']}\n"
        text += f"🏦 {c['bank_name']}\n"
        text += f"💳 `{c['card_number']}`\n"
        text += f"👤 {c['card_holder']}\n"
        text += "─" * 20 + "\n"
...
def monitor_system():
    while True:
        try:
            ...
            # پاکسازی سندباکس‌های قدیمی
            for f in os.listdir(DIRS['SANDBOX']):
                path = os.path.join(DIRS['SANDBOX'], f)
                if os.path.isdir(path) and now - os.path.getmtime(path) > 86400:  # 24 ساعت
                    try:
                        shutil.rmtree(path)
                    except:
                        pass
            
            # به روز رسانی لود بالانس
            load_balancer._init_machines()
