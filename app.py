...
# ==================== تنظیمات اصلی ====================
BOT_TOKEN = "8723263125:AAG_dGKL2MOmh1SrYr8pSt_Goj-4LTtCVC4"
ADMIN_ID = 327855654
...
REQUIRED_REFERRALS = 0  # ===== رفرال برای ثبت لینک اجباری نیست =====
...
REFERRAL_BOOST_THRESHOLD = 100  # ===== آستانه رفرال برای افزایش بازدید =====
REFERRAL_BOOST_MULTIPLIER = 2  # ===== ضریب افزایش بازدید =====
...
class DistributedUltraCache:
    def __init__(self, max_size=5_000_000, ttl_seconds=300, shard_count=500):
...
class UltraScalableDatabase:
    def __init__(self, db_path="bot_database.db", shard_count=500):
        self.db_path = db_path
        self.shard_count = shard_count
        self.cache = DistributedUltraCache(max_size=5_000_000, ttl_seconds=300, shard_count=500)
        
        self.shard_paths = []
        for i in range(shard_count):
            shard_dir = f"shards_{i // 50}"
    ...
    def get_boost_multiplier(self, referral_count: int) -> int:
        ...
        if referral_count >= REFERRAL_BOOST_THRESHOLD * 10:  # 1000+
            return REFERRAL_BOOST_MULTIPLIER * 5  # 10x
        elif referral_count >= REFERRAL_BOOST_THRESHOLD * 5:  # 500+
            return REFERRAL_BOOST_MULTIPLIER * 3  # 6x
        elif referral_count >= REFERRAL_BOOST_THRESHOLD * 2:  # 200+
            return REFERRAL_BOOST_MULTIPLIER * 2  # 4x
        elif referral_count >= REFERRAL_BOOST_THRESHOLD:  # 100+
            return REFERRAL_BOOST_MULTIPLIER  # 2x
        else:
            return 1
        ...
        async with self.get_connection(user_id) as conn:
            ...
            if referred_by:
                ...
                async with self.get_connection(referred_by) as conn2:
                    ...
                    await self.cache.set(f"ref_count_{referred_by}", new_count, ttl=60)
                    
                    # بروزرسانی total_referrals
    ...
    async def distribute_links(self, user_id: int, count: int = DAILY_INTERACTIONS_REQUIRED) -> List[Dict]:
        ...
        async with self.get_connection(user_id) as conn:
            # دریافت ضریب بازدید کاربر
            ref_count = await self.get_user_referrals_count(user_id)
            boost = self.get_boost_multiplier(ref_count)
            actual_count = count * boost  # افزایش تعداد لینک‌های مورد نیاز بر اساس ضریب
            ...
            cursor.execute("""
                ...
                WHERE yl.user_id != ? AND yl.is_active = 1 AND u.is_subscribed = 1
...
class YouTubeEarningBot:
    def __init__(self, token: str):
        self.token = token
        self.db = UltraScalableDatabase(shard_count=500)
        ...
        logger.info("🚀 ربات با معماری میکروسرویس راه‌اندازی شد")
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == 'change_language':
            ...
            # ===== رفرال برای ثبت لینک اجباری نیست =====
            # فقط بررسی محدودیت روزانه
