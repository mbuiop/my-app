...
class AIClient:
    ...
    async def _call(self, system_prompt, user_content, max_tokens=1024, retries=1):
        if not self.is_configured():
            logger.error("AI call skipped: OPENAI_API_KEY is not set (still contains the placeholder text).")
        ...
        for attempt in range(retries + 1):
            try:
                ...
                async with session.post("https://api.openai.com/v1/chat/completions", headers=headers, json=body) as resp:
                    ...
                    if resp.status != 200:
                        logger.error(f"OpenAI API error (status {resp.status}, attempt {attempt+1}): {raw_text[:2000]}")
                        last_error = f"API error {resp.status}: {raw_text[:300]}"
                        # Auth/config errors won't fix themselves on retry — stop immediately.
                    ...
                    if not choices:
                        logger.error(f"OpenAI API returned no choices: {raw_text[:1000]}")
                    ...
                    text = text.strip()
                    if not text:
                        last_error = "AI returned empty text"
                        await asyncio.sleep(1.0)
                        continue
                    return text, None
                ...
                logger.error(f"OpenAI API exception (attempt {attempt+1}): {e}")
                last_error = str(e)
                await asyncio.sleep(1.0)
...
class UTYOBot:
    ...
    async def _safe_edit(self, query, text, reply_markup=None):
        """Edit a callback's original message whether it's plain text or a photo-with-caption."""
        try:
            ...
            logger.warning(f"_safe_edit failed, falling back to new message: {e}")
            try:
                await query.message.reply_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
            except Exception as e2:
                logger.error(f"_safe_edit fallback also failed: {e2}")
    ...
    async def _handle_chart_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if analysis:
            ...
            error_text = self._t(user_id, 'chart_error')
            if user_id in ADMIN_IDS and err:
                error_text += f"\n\n🔧 Debug: `{err}`"
    ...
    async def admin_verify_approve_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not p:
            await self._safe_edit(query, self._ta('admin_not_found'))
            return
        ...
        try:
            ...
            pass
        await self._safe_edit(query, self._ta('admin_approved_note', p['user_id']))
    ...
    async def admin_verify_reject_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            pass
        await self._safe_edit(query, self._ta('admin_rejected_note', p['user_id']))
